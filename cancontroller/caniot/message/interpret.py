from cancontroller.caniot.message import TelemetryMessage, AttributeResponse, AttributeResponse, CaniotMessage, ErrorMessage
...
def interpret_response(msgid: MsgId, buffer: BufferType):
    if msgid.is_error():
        return ErrorMessage(msgid, buffer)
    elif msgid.is_response():
