...
from cancontroller.can.models import gen_garage_can_command, MsgId, DeviceId, ControllerMessageBuilder, \
...
logging.basicConfig()
logging.getLogger("can.interfaces.socketcan.socketcan").setLevel(logging.WARNING)
...
class CanController(model_pb2_grpc.CanControllerServicer):
    ...
    def handle_can_message(self, msg: can.Message):
        msgid = MsgId.from_int(msg.arbitration_id)
        can_logger.debug(f"RX {msgid} payload[{len(msg.data)}] : {msg.data}")
    ...
    async def SendGarage(self, request: model_pb2.GarageCommand, context) -> model_pb2.GarageResponse:
        ...
        self.send(gen_garage_can_command(request.command, 0))  # dev
        self.send(gen_garage_can_command(request.command, 1))  # prod
